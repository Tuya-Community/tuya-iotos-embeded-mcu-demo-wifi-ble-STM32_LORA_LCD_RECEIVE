...
void RadioInit(void)
{
    ...
    SX126xSetRegulatorMode( USE_DCDC);//USE_LDO
   ...
   // TimerInit( &TxTimeoutTimer, RadioOnTxTimeoutIrq );//TX超时中断
   // TimerInit( &RxTimeoutTimer, RadioOnRxTimeoutIrq );//RX超时中断
}
void RadioSetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
   ...
   // TimerStart( &RxTimeoutTimer );//定时器开启，暂时不用中断  所以不用
}
                         ...
                         bool iqInverted, bool rxContinuous )
{
    ...
    switch( modem )
    {
        case MODEM_FSK:
            ...
            SX126x.PacketParams.Params.Gfsk.AddrComp = RADIO_ADDRESSCOMP_FILT_OFF;
            SX126x.PacketParams.Params.Gfsk.HeaderType = ( fixLen == true ) ? RADIO_PACKET_FIXED_LENGTH : RADIO_PACKET_VARIABLE_LENGTH;
            SX126x.PacketParams.Params.Gfsk.PayloadLength = 10;//MaxPayloadLength;
            if( crcOn == true )
            {
...
/*
	...
	if(HAL_GPIO_ReadPin(DeviceSel_GPIO_Port,DeviceSel_Pin)==GPIO_PIN_SET)
    {
        return SX1261;
    }
    else
